...
import {
  DEFAULT_PROFESSOR_SORT_OPTION,
  PROFESSOR_POPULATE_OPTIONS,
  PROFESSOR_SORT_OPTIONS,
} from './constants/professor.constants';
...
export class ProfessorService {
  ...
  constructor(
    ...
    this.professorModel = this.userModel.discriminators?.[
      UserRole.PROFESSOR
    ] as PaginateModel<ProfessorDocument>;
  }

  async createProfessor(
    createProfessorDto: CreateProfessorRequestDto,
  ): Promise<ProfessorResponseDto> {
    const professor = new this.professorModel({
      role: UserRole.PROFESSOR,
      password: this.generateProfessorPassword(
        createProfessorDto.documentNumber,
      ),
      ...createProfessorDto,
    });
    ...
    await this.academicProgramService.validateAcademicProgramExists(
      createProfessorDto.academicProgram,
    );
  ...
  async getProfessorsByCriteria(
    professorFilter: ProfessorFilterDto,
  ): Promise<PaginatedResult<ProfessorResponseDto>> {
  ...
  async updateProfessor(
    professorId: string,
    updateProfessorDto: UpdateProfessorRequestDto,
  ): Promise<ProfessorResponseDto> {
    ...
    if (updateProfessorDto.academicProgram) {
      await this.academicProgramService.validateAcademicProgramExists(
        updateProfessorDto.academicProgram,
      );
    }
  ...
  private buildFilter(
    ...
    const filter: Record<string, any> =
      this.userService.buildFilter(professorFilter);
