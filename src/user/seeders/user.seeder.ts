...
export class UserSeeder implements OnModuleInit {
  ...
  private async seedSuperAdmin() {
    ...
    try {
      ...
      this.logger.log(
        `Super admin user created successfully: ${adminUser.email}`,
      );
    } catch (error) {
  ...
  private buildAdminUser(): CreateUserDto {
    ...
    const adminDocumentNumber = this.configService.get<string>(
      'SUPER_ADMIN_DOCUMENT_NUMBER',
    );

    if (!adminEmail || !adminPassword || !adminPhone || !adminDocumentNumber) {
      this.logger.error(
        'Missing environment variables for super admin user creation.',
      );
      throw new Error(
        'Missing environment variables for super admin user creation.',
      );
