...
import {
  CompanyMentor,
  CompanyMentorDocument,
} from './schemas/company-mentor.schema';
...
import {
  COMPANY_MENTOR_POPULATE_OPTIONS,
  COMPANY_MENTOR_SORT_OPTIONS,
  DEFAULT_COMPANY_MENTOR_SORT_OPTION,
} from './constants/company-mentor.constants';
...
export class CompanyMentorService {
  ...
  constructor(
    ...
    this.companyMentorModel = this.userModel.discriminators?.[
      UserRole.COMPANY_MENTOR
    ] as PaginateModel<CompanyMentorDocument>;
  }

  async createCompanyMentor(
    createCompanyMentorDto: CreateCompanyMentorRequestDto,
  ): Promise<CompanyMentorResponseDto> {
    const companyMentor = new this.companyMentorModel({
      role: UserRole.COMPANY_MENTOR,
      password: this.generateCompanyMentorPassword(
        createCompanyMentorDto.documentNumber,
      ),
      ...createCompanyMentorDto,
    });

    await this.userService.validateUniqueFields(createCompanyMentorDto);
    await this.companyService.validateCompanyExists(
      createCompanyMentorDto.company,
    );
  ...
  async getCompanyMentorsByCriteria(
    ...
    return this.companyMentorMapper.toCompanyMentorPaginatedResponseDto(
      companyMentors,
    );
  }
  ...
  async updateCompanyMentor(
    ...
    const companyMentor =
      await this.companyMentorModel.findById(companyMentorId);
    if (!companyMentor) {
    ...
    if (updateCompanyMentorDto.company) {
      await this.companyService.validateCompanyExists(
        updateCompanyMentorDto.company,
      );
    }
