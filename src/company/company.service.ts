...
export class CompanyService {
    ...
    async updateCompany(companyId: string, dto: UpdateCompanyRequestDto): Promise<CompanyResponseDto> {
        const company = await this.companyModel.findById(companyId);
        if (!company) throw new CompanyNotFoundException();
    ...
    async updateCompanyLogo(companyId: string, file: Express.Multer.File): Promise<CompanyResponseDto> {
        console.log('Updating company logo for companyId:', companyId, 'with file:', file);
        const company = await this.companyModel.findById(companyId);
        if (!company) throw new CompanyNotFoundException();
    ...
    async getByCriteria(filter: CompanyFilterDto): Promise<PaginatedResult<CompanyResponseDto>> {
        const query = this.buildFilterQuery(filter);
        const sort = getSort(COMPANY_SORT_OPTIONS, DEFAULT_COMPANY_SORT_OPTION, filter.sortBy, filter.sortOrder);

        const result: PaginateResult<CompanyDocument> = await this.companyModel.paginate(query, {
            page: filter.page,
            limit: filter.limit,
            sort,
            populate: [
    ...
    private async validateUniqueFields(dto: UpdateCompanyRequestDto | CreateCompanyRequestDto, excludeId?: string): Promise<void> {
        const checks = [
            { field: 'name', value: dto.name, Exception: CompanyNameAlreadyExistsException },
            { field: 'corporateName', value: dto.corporateName, Exception: CompanyCorporateNameAlreadyExistsException },
            { field: 'nit', value: dto.nit, Exception: CompanyNitAlreadyExistsException },
            { field: 'phone', value: dto.phone, Exception: CompanyPhoneAlreadyExistsException },
        ];
    ...
    private async validateReferencedFields(dto: UpdateCompanyRequestDto | CreateCompanyRequestDto): Promise<void> {
        ...
        if (dto.associatedAcademicPrograms) {
            await this.academicProgramService.validateManyAcademicProgramsExist(dto.associatedAcademicPrograms);
        }
    }

    private async populateAndMapCompany(company: CompanyDocument): Promise<CompanyResponseDto> {
